...
insert into public.products(id,name,bottles_per_crate,full_crate_price,bottles_returnable,crate_type_id,empty_family) values
 ('10000000-0000-4000-8000-000000000031','Receive test',12,0,false,'8a070e73-7dfe-5e54-a456-e954843fc700','same family'),
 ('10000000-0000-4000-8000-000000000032','Missing empties',24,0,false,'d8175ef0-8ca7-5a1b-a553-b7cc86023797','same family');
...
select public.receive_stock('20000000-0000-4000-8000-000000000031','10000000-0000-4000-8000-000000000031',2,5,10,12,'8a070e73-7dfe-5e54-a456-e954843fc700','2026-09-10');
...
do $$ declare n numeric; begin
  ...
  if (select quantity from public.empty_crate_stock where crate_type_id='8a070e73-7dfe-5e54-a456-e954843fc700') <> 8 then raise exception 'Wrong empty count'; end if;
  foreach n in array array[0,-1,1.5,9,'NaN'::numeric,'Infinity'::numeric] loop
    begin
      perform public.receive_stock(gen_random_uuid(),'10000000-0000-4000-8000-000000000031',n,29,8,12,'8a070e73-7dfe-5e54-a456-e954843fc700','2026-09-10');
      raise exception 'Invalid or insufficient quantity accepted: %', n;
    exception when invalid_parameter_value then null; end;
  end loop;
  begin
    perform public.receive_stock(gen_random_uuid(),'10000000-0000-4000-8000-000000000032',1,0,0,24,'d8175ef0-8ca7-5a1b-a553-b7cc86023797','2026-09-10');
    raise exception 'Family compatibility inferred';
  exception when invalid_parameter_value then null; end;
  begin
    perform public.receive_stock(gen_random_uuid(),'10000000-0000-4000-8000-000000000031',1,5,10,12,'8a070e73-7dfe-5e54-a456-e954843fc700','2026-09-10');
    raise exception 'Stale review accepted';
  exception when invalid_parameter_value then null; end;
  begin
    perform public.receive_stock('20000000-0000-4000-8000-000000000031','10000000-0000-4000-8000-000000000031',3,29,8,12,'8a070e73-7dfe-5e54-a456-e954843fc700','2026-09-10');
...
do $$ begin
  begin
    perform public.receive_stock('20000000-0000-4000-8000-000000000033','10000000-0000-4000-8000-000000000031',1,29,8,12,'8a070e73-7dfe-5e54-a456-e954843fc700','2026-09-10');
    raise exception 'Expected stock failure';
  exception when check_violation then null; end;
  if (select quantity from public.empty_crate_stock where crate_type_id='8a070e73-7dfe-5e54-a456-e954843fc700') <> 8 then raise exception 'Empty deduction did not roll back'; end if;
...
insert into public.empty_crate_stock(crate_type_id,quantity) values ('d8175ef0-8ca7-5a1b-a553-b7cc86023797',1);
set local role authenticated;
select public.receive_stock('20000000-0000-4000-8000-000000000034','10000000-0000-4000-8000-000000000032',1,0,1,24,'d8175ef0-8ca7-5a1b-a553-b7cc86023797','2026-09-10');
select set_config('request.jwt.claim.sub','00000000-0000-4000-8000-000000000032',true);
do $$ begin
  begin
    perform public.receive_stock(gen_random_uuid(),'10000000-0000-4000-8000-000000000031',1,29,8,12,'8a070e73-7dfe-5e54-a456-e954843fc700','2026-09-10');
    raise exception 'Non-owner authorized';
  exception when insufficient_privilege then null; end;
  begin
    perform private.receive_stock(gen_random_uuid(),'10000000-0000-4000-8000-000000000031',1,29,8,12,'8a070e73-7dfe-5e54-a456-e954843fc700','2026-09-10');
...
do $$ begin
  ...
  if (select quantity from public.empty_crate_stock where crate_type_id='d8175ef0-8ca7-5a1b-a553-b7cc86023797') <> 0 then raise exception 'Empty crates did not reach zero'; end if;
end $$;
rollback;
