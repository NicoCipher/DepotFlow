...
insert into public.products(id,name,bottles_per_crate,full_crate_price,bottles_returnable,crate_type,empty_family) values
 ('10000000-0000-4000-8000-000000000051','Exact A',12,0,false,'exact A','same family'),
 ('10000000-0000-4000-8000-000000000052','Exact B',24,0,false,'exact B','same family'),
 ('10000000-0000-4000-8000-000000000053','Another A',12,0,false,'exact A','same family');
insert into public.empty_crate_stock values ('existing / type',3);
...
select pg_temp.check_empty((select quantity is null from public.known_empty_crates where crate_type='exact A'),'Unrecorded incorrectly zero');
select pg_temp.check_empty((select quantity=3 from public.known_empty_crates where crate_type='existing / type'),'Stock-only type missing');
select public.set_empty_crate_count('20000000-0000-4000-8000-000000000051','exact A',13,'2024-02-29',null);
select public.set_empty_crate_count('20000000-0000-4000-8000-000000000051','exact A',13,'2024-02-29',null);
select pg_temp.check_empty((select quantity=13 from public.empty_crate_stock where crate_type='exact A'),'Positive count failed');
...
select public.set_empty_crate_count('20000000-0000-4000-8000-000000000052','exact A',5,'2024-03-01',13);
select pg_temp.check_empty((select quantity=5 from public.empty_crate_stock where crate_type='exact A'),'Count added instead of replaced');
...
select public.receive_stock('20000000-0000-4000-8000-000000000053','10000000-0000-4000-8000-000000000051',2,0,5,12,'exact A','2024-03-01');
select pg_temp.check_empty((select quantity=3 from public.empty_crate_stock where crate_type='exact A'),'Receiving did not deduct exact A');
...
select pg_temp.check_empty((select quantity is null from public.known_empty_crates where crate_type='exact B'),'Receiving inferred family compatibility');
-- Retrying a previous count must not rewind a later receiving deduction.
select public.set_empty_crate_count('20000000-0000-4000-8000-000000000052','exact A',5,'2024-03-01',13);
select pg_temp.check_empty((select quantity=3 from public.empty_crate_stock where crate_type='exact A'),'Retry overwrote receipt deduction');
do $$ declare n numeric; d date; t text; begin
  foreach n in array array[-1,0.5,2147483648,'NaN'::numeric,'Infinity'::numeric,null] loop
    begin
      perform public.set_empty_crate_count(gen_random_uuid(),'exact A',n,'2024-03-02',3);
  ...
  foreach d in array array[null,'infinity'::date,'-infinity'::date,'10000-01-01'::date] loop
    begin
      perform public.set_empty_crate_count(gen_random_uuid(),'exact A',0,d,3);
      raise exception 'Invalid date accepted';
    exception when invalid_parameter_value then null; end;
  end loop;
  foreach t in array array[null,'','unknown','exact a','same family',' exact A '] loop
    begin
  ...
  begin
    perform public.set_empty_crate_count(gen_random_uuid(),'exact A',0,'2026-02-30',3);
    raise exception 'Impossible calendar date accepted';
  exception when datetime_field_overflow then null; end;
  begin
    perform public.set_empty_crate_count('20000000-0000-4000-8000-000000000052','exact A',6,'2024-03-01',3);
    raise exception 'Changed retry quantity accepted';
  exception when invalid_parameter_value then null; end;
  begin
    perform public.set_empty_crate_count('20000000-0000-4000-8000-000000000052','exact B',5,'2024-03-01',null);
    raise exception 'Changed retry type accepted';
  exception when invalid_parameter_value then null; end;
  begin
    perform public.set_empty_crate_count('20000000-0000-4000-8000-000000000052','exact A',5,'2024-03-02',3);
    raise exception 'Changed retry date accepted';
  exception when invalid_parameter_value then null; end;
  begin
    perform public.set_empty_crate_count(gen_random_uuid(),'exact A',0,'2024-03-02',5);
    raise exception 'Stale review overwrote receipt';
  exception when invalid_parameter_value then null; end;
  begin
    perform public.set_empty_crate_count(gen_random_uuid(),'exact A',0,'2024-03-02',null);
    raise exception 'Initial review overwrote existing count';
  exception when invalid_parameter_value then null; end;
  begin
    perform public.receive_stock(gen_random_uuid(),'10000000-0000-4000-8000-000000000052',1,0,0,24,'exact B','2024-03-02');
  ...
  begin
    insert into public.empty_crate_stock values ('direct',1);
...
do $$ begin
  begin
    perform public.set_empty_crate_count('20000000-0000-4000-8000-000000000054','exact A',0,'2024-03-02',3);
    raise exception 'Expected existing count failure';
  exception when check_violation then null; end;
  begin
    perform public.set_empty_crate_count('20000000-0000-4000-8000-000000000055','exact B',0,'2024-03-02',null);
    raise exception 'Expected initial count failure';
  exception when check_violation then null; end;
end $$;
select pg_temp.check_empty((select quantity=3 from public.empty_crate_stock where crate_type='exact A'),'Update did not roll back');
select pg_temp.check_empty(not exists(select 1 from public.empty_crate_stock where crate_type='exact B'),'Initial insertion did not roll back');
...
set local role authenticated;
select public.set_empty_crate_count('20000000-0000-4000-8000-000000000054','exact A',0,'2024-03-02',3);
select public.set_empty_crate_count('20000000-0000-4000-8000-000000000055','exact B',0,'2024-03-02',null);
select pg_temp.check_empty((select quantity=0 from public.known_empty_crates where crate_type='exact B'),'Confirmed zero still unrecorded');
...
select public.set_empty_crate_count(gen_random_uuid(),'existing / type',1,'2024-03-02',3);
...
do $$ begin
  begin
    perform public.set_empty_crate_count(gen_random_uuid(),'exact A',1,'2024-03-02',0);
    raise exception 'Non-owner counted';
  exception when insufficient_privilege then null; end;
  begin
    perform private.set_empty_crate_count(gen_random_uuid(),'exact A',1,'2024-03-02',0);
